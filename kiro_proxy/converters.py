...
def _detect_image_format(media_type: str) -> str:
    """从 media_type 检测图片格式"""
...
async def _download_image_url(url: str) -> Tuple[str, str]:
    """异步下载 URL 图片并转为 base64（使用连接池，不阻塞事件循环）
    
    Returns:
        (format, base64_data) or ("", "") on failure
    """
    try:
        resp = await http_pool.short_client.get(url, headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
        if resp.status_code == 200:
            content_type = resp.headers.get("content-type", "image/jpeg")
            fmt = _detect_image_format(content_type)
            data = base64.b64encode(resp.content).decode("utf-8")
            return fmt, data
        else:
            print(f"[Image] URL 图片下载失败: HTTP {resp.status_code}")
            return "", ""
    except Exception as e:
        print(f"[Image] URL 图片下载失败: {e}")
        return "", ""


async def extract_images_from_content(content) -> Tuple[str, List[dict]]:
    """从消息内容中提取文本和图片
    
    支持格式：
    - Anthropic base64: {"type": "image", "source": {"type": "base64", "media_type": "...", "data": "..."}}
    - Anthropic URL: {"type": "image", "source": {"type": "url", "url": "https://..."}}
    - Anthropic file: {"type": "file", "source": {"type": "base64", "media_type": "image/...", "data": "..."}}
    - OpenAI data URI: {"type": "image_url", "image_url": {"url": "data:image/...;base64,..."}}
    - OpenAI URL: {"type": "image_url", "image_url": {"url": "https://..."}}
        ...
        if isinstance(block, str):
            ...
            if block_type == "text":
                ...
                if source_type == "base64":
                    ...
                    if data:
                        fmt = _detect_image_format(media_type)
                        images.append({
                            "format": fmt,
                            "source": {"bytes": data}
                        })
                    ...
                    url = source.get("url", "")
                    if url:
                        fmt, data = await _download_image_url(url)
                        if data:
                            images.append({
                                "format": fmt,
                                "source": {"bytes": data}
